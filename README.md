...
```mermaid
graph TD
    User([Student / Browser]) -->|HTTPS / WSS| CDN[Vercel Global Edge CDN]
    CDN -->|React SPA Frontend| ReactApp[NoteShare React 18 App]
    ...
    subgraph Backend Infrastructure Render.com
        WAF -->|Block Scrapers & Honeypot Traps| ExpressServer[Express.js Node Server]
        ExpressServer -->|24/7 Keep-Alive| HealthCheck[cron-job.org /api/health]
        ...
        ExpressServer -->|60s Presigned S3 URLs| CloudflareR2[Cloudflare R2 Storage]
        ExpressServer -->|2-Step OTP & Alerts| ResendAPI[Resend Email API]
...
## 5-Layer Anti-Scraping & Cyber Security
...
| `POST` | `/api/feedback` | Submit student review & rating | Optional / Auth |
